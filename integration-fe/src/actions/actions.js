...
/*
function saveInstance(instancePayload) {
  debugger;
  const integrationConfigService = SYMPHONY.services.subscribe('integration-config');
  const promisedInstance = integrationConfigService.createConfigurationInstance(
    configurationId, instancePayload);
  return (dispatch) => {
    Promise.resolve(promisedInstance)
    .then((data) => {
...
function buildInstancePayload() {
  ...
  return (dispatch, getState) => {
    ...
    const optionalProperties = `{"owner": "${state.userId}", "streams": [${opStreams}],
    "streamType": "${state.entities.instance.streamType}"}`;
    // build payload
    const payload = {
      configurationId: state.configurationId,
      name: state.entities.instance.name,
    ...
    dispatch(saveInstance(payload));
  };
}

function addRoomMemberShipSuccess(response) {
  debugger;
  return (dispatch) => {
    dispatch(buildInstancePayload());
  };
}

export function addRoomMemberShip(streams) {
  const promises = [];
  const streamService = SYMPHONY.services.subscribe('stream-service');
  debugger;
  for (const stream in streams) {
  ...
  return (dispatch) => {
    Promise.all(promises).then((values) => {
      dispatch(addRoomMemberShipSuccess(values));
    }, (err) => {
      dispatch(setError(err));
    });
  };
